...
class UnitTests(unittest.TestCase):
 def prepare(self,text,kind='auto',suffix='.md',limit=60000):
  with tempfile.TemporaryDirectory() as d:
   p=Path(d)/('test'+suffix);p.write_text(text,encoding='utf-8');return pre.prepare(p,True,d,kind,limit)
 ...
 def test_large_context_model_uses_larger_unit_limit(self):
  text='甲。'*42500;r=self.prepare(text,'article',limit=90000);self.assertEqual(len(r['chapters']),1);self.assertEqual(r['chapters'][0]['text'],text)
