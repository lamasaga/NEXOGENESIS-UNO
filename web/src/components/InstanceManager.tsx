...
function InstanceRow({ instance, busy, editing, removing, onEdit, onCancel, onSwitch, onRename, onStartRemove, onRemove, name, setName }: {
  ...
  return <article className={`instance-manager__item${instance.active ? " is-active" : ""}`}>
    <div><strong>{instance.name}</strong>{instance.active ? <span>当前</span> : null}<small>{instance.card_count ?? '未知'} 张卡片 · {instance.legacy ? "兼容目录" : "独立实例"}</small></div>
    {editing ? <div className="instance-manager__edit"><input aria-label="实例名称" value={name} maxLength={80} onChange={(event) => setName(event.target.value)} /><button type="button" disabled={busy || !name.trim()} onClick={onRename}>保存</button><button type="button" onClick={onCancel}>取消</button></div> : removing ? <div className="instance-manager__remove"><small>只移出本应用登记，磁盘中的知识目录不会删除。</small><button type="button" disabled={busy} onClick={onRemove}>确认移出</button><button type="button" onClick={onCancel}>取消</button></div> : <div className="instance-manager__actions">{!instance.active ? <button type="button" disabled={busy || (instance.status !== undefined && instance.status !== 'available')} onClick={onSwitch}>切换</button> : null}<button type="button" disabled={busy || (instance.status !== undefined && instance.status !== 'available')} onClick={onEdit}>改名</button>{!instance.active ? <button type="button" disabled={busy} onClick={onStartRemove}>移出</button> : null}</div>}
  </article>;
}
